...
void _DRV_USART1_BufferQueueRxTasks(void)
{
    ...
    if(bufferObj != NULL)
    {
        ...
        while((PLIB_USART_ReceiverDataIsAvailable(USART_ID_3))
              && (bufferObj->nCurrentBytes < bufferObj->size))
        {
            bufferObj->buffer[bufferObj->nCurrentBytes] = PLIB_USART_ReceiverByteReceive(USART_ID_3);
            bufferObj->nCurrentBytes++;
        }
...
void _DRV_USART1_BufferQueueTxTasks(void)
{
    ...
    if(dObj->queueWrite != NULL)
    {
        ...
        while((!PLIB_USART_TransmitterBufferIsFull(USART_ID_3))
              && (bufferObj->nCurrentBytes < bufferObj->size))
        {
            PLIB_USART_TransmitterByteSend(USART_ID_3, bufferObj->buffer[bufferObj->nCurrentBytes]);
            bufferObj->nCurrentBytes++;
        }
    }
