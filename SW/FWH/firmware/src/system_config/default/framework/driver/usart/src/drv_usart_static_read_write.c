...
// *****************************************************************************
// *****************************************************************************
size_t DRV_USART1_Read(void * destination,const size_t nBytes)
{
    DRV_USART_OBJ *dObj = (DRV_USART_OBJ*)NULL;
    uint8_t * data;
    size_t count = 0;
    ...
    {
        while((PLIB_USART_ReceiverDataIsAvailable(USART_ID_3)) && (count < nBytes))
        {
            /* We need to check for errors. Store the error
               in the client error field. */
            dObj->error = PLIB_USART_ErrorsGet(USART_ID_3);
            ...
            /* This is not a blocking implementation. We read
               the hardware till the FIFO is empty. */
            data[count] = PLIB_USART_ReceiverByteReceive(USART_ID_3);
            count ++;
        }
        /* Unlock the mutex */
        OSAL_MUTEX_Unlock(&(dObj->mutexDriverInstance));
...
size_t DRV_USART1_Write(void * source, const size_t nBytes)
{
    DRV_USART_OBJ *dObj = (DRV_USART_OBJ*)NULL;
    size_t count = 0;
    uint8_t * data;
    ...
    {
        while((!PLIB_USART_TransmitterBufferIsFull(USART_ID_3)) && (count < nBytes))
        {
            /* This is not a blocking implementation. We write
               to the hardware till the FIFO is full. */
            PLIB_USART_TransmitterByteSend(USART_ID_3, data[count]);
            count ++;
...
// *****************************************************************************
// *****************************************************************************
size_t DRV_USART2_Read(void * destination,const size_t nBytes)
{
    DRV_USART_OBJ *dObj = (DRV_USART_OBJ*)NULL;
    uint8_t * data;
    size_t count = 0;
    ...
    {
        while((PLIB_USART_ReceiverDataIsAvailable(USART_ID_2)) && (count < nBytes))
        {
            /* We need to check for errors. Store the error
               in the client error field. */
            ...
            /* This is not a blocking implementation. We read
               the hardware till the FIFO is empty. */
            data[count] = PLIB_USART_ReceiverByteReceive(USART_ID_2);
            count ++;
        }
        /* Unlock the mutex */
        OSAL_MUTEX_Unlock(&(dObj->mutexDriverInstance));
...
size_t DRV_USART2_Write(void * source, const size_t nBytes)
{
    DRV_USART_OBJ *dObj = (DRV_USART_OBJ*)NULL;
    size_t count = 0;
    uint8_t * data;
    ...
    {
        while((!PLIB_USART_TransmitterBufferIsFull(USART_ID_2)) && (count < nBytes))
        {
            /* This is not a blocking implementation. We write
               to the hardware till the FIFO is full. */
            PLIB_USART_TransmitterByteSend(USART_ID_2, data[count]);
            count ++;
