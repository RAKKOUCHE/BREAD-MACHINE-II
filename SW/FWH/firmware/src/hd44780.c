 ...
 */
#define LCD_PORT_DATA PORT_CHANNEL_A

/**
 * \brief Positionne le bus de data en entrée.
 */
#define LCD_DATA_IN() (TRISASET = LCD_DATA)// GPIO_PortInputEnable (LCD_PORT_DATA, LCD_DATA)

/**
 * \brief Positionne le bus de data en sortie.
 */
#define LCD_DATA_OUT() (TRISACLR = LCD_DATA)

/**
 * \brief Positionne à zéro le port de données.
 */
#define LCD_CLEAR_DATA() (LATACLR = LCD_DATA)
...
static void vLCD_Set_Interface(const bool is8Bits)
{
    LCD_DATA_OUT();
    LCD_CLEAR_DATA();
    LATASET = LCD_FUNCTION_SET | (is8Bits ? LCD_8_BITS : 0);
    vLCD_Enable();
}
